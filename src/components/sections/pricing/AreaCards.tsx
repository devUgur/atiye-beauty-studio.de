    ...
    return (
        <div className="space-y-12 max-w-4xl mx-auto">
            {sortedCategories.map((category, index) => (
                ...
                >
                    {/* Category Header */}
                    <div className="mb-6 pb-3 border-b border-stone-200 dark:border-stone-800">
                        <h3 className="font-serif text-2xl md:text-3xl font-medium text-stone-900 dark:text-white">
                            {category.title}
                        </h3>
                    </div>

                    {/* Modern Price List Table */}
                    <div className="bg-white/40 dark:bg-stone-900/40 backdrop-blur-sm border border-stone-200/60 dark:border-stone-800/60 rounded-none overflow-hidden">
                        <div className="divide-y divide-stone-200/60 dark:divide-stone-800/60">
                            {category.items.map((item) => (
                                <div
                                    key={item.name}
                                    className="group flex items-center justify-between px-4 sm:px-6 py-4 hover:bg-white/60 dark:hover:bg-stone-800/60 transition-[background-color] duration-200"
                                >
